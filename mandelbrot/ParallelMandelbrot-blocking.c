/************************************************
 CS789 - Parallel Programming 
	...
	A simple MPI program to create Mandelbrot picture using blocking send/recv method. Master
	receives calculations done by each worker in the correct sequence by waiting for each
...
int main(int argc, char *argv[]) {
	...
	if (rank == 0) {
		...
		for(worker = 1; worker < size; worker++) {
			...
			block_height = disp_height/(size - 1) + (worker <= disp_height%(size-1) ? 1 : 0);
			...
			MPI_Recv(&block[0], block_height * disp_width, MPI_INT, worker, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			for (y=0; y<block_height; y++) {
				for (x=0; x<disp_width; x++) {
					pic[((y + ((worker-1)*block_height)) * disp_width) + x] = block[(y * disp_width) + x];
				}
			}
		}
